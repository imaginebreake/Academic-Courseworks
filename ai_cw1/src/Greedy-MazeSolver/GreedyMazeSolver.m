...
% QUEUE: [0/1, X val, Y val, Parent X val, Parent Y val, g(n), h(n)]
xNode = xStart;
yNode = yStart;
QUEUE = [];
QUEUE_COUNT = 1;
...
while ((xNode ~= xTarget || yNode ~= yTarget) && NoPath == 1)
    ...
    for i = 1:exp_count
        ...
        for j = 1:QUEUE_COUNT
            ...
            if (exp(i, 1) == QUEUE(j, 2) && exp(i, 2) == QUEUE(j, 3))
                % update the h(n) of node on QUEUE
    ...
    % Greedy : find the node in QUEUE with the smallest h(n), returned by min_hn
